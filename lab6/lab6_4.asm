...
buforForReading db 4 dup(0)
buforForPath dd 128 dup (0)
buforForFileNameDat db "\—uk.dat",0
buforForFileNameTxt db "\—uk.txt",0
buforForAddZero db 1 dup(0)
buforForAddSeparator db "-",0
...
_TEXT SEGMENT
main proc
    ;Napisz program, wczytuj1cy liczby z pliku TwojeNazwisko.dat (zad2) i kopiuj1cy je do pliku TwojeNazwisko.txt (1 pkt) Liczby w nowym pliku powinny by?apisane w formacie tekstowym (1pkt) i rozdzielone separatorem (2pkt). Znak-separator liczb zale?ny od wariantu wskazanego przez prowadz1cego.

    invoke GetCurrentDirectoryA,128,OFFSET buforForPath
    invoke lstrcatA,OFFSET buforForPath,OFFSET buforForFileNameDat
    invoke CreateFileA,OFFSET buforForPath, GENERIC_READ OR GENERIC_WRITE,0,0,OPEN_EXISTING,0,0
    mov fileHandlerForReading, EAX

    invoke GetCurrentDirectoryA,128,OFFSET buforForPath
    invoke lstrcatA,OFFSET buforForPath,OFFSET buforForFileNameTxt
    invoke CreateFileA,OFFSET buforForPath, GENERIC_READ OR GENERIC_WRITE,0,0,CREATE_ALWAYS,0,0
    mov fileHandlerForWriting, EAX
    mov ECX, 20
    L1:
        mov counter, ECX
        ;mov EDX, OFFSET buforForWriting
        invoke ReadFile, fileHandlerForReading, offset buforForReading, 4, numberOfBytesWritten, 0
        mov EBX, OFFSET buforForReading
        mov EDX, offset buforForWriting

        mov ESI, OFFSET buforForReading

        invoke dwtoa, [ESI], offset buforForWriting

        invoke lstrcatA,OFFSET buforForWriting,OFFSET buforForAddZero
        invoke lstrcatA,OFFSET buforForWriting,OFFSET buforForAddSeparator
        invoke WriteFile, fileHandlerForWriting, offset buforForWriting, 4, numberOfBytesWritten, 0
